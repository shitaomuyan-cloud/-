...
const CFG_MAP={sign_lo:"cfg-sign-lo",sign_hi:"cfg-sign-hi",lottery_cost:"cfg-lottery-cost",lottery_lo:"cfg-lottery-lo",lottery_hi:"cfg-lottery-hi",lottery_win_rate:"cfg-lottery-win",robbery_lo:"cfg-robbery-lo",robbery_hi:"cfg-robbery-hi",robbery_rate:"cfg-robbery-rate",mute_cost:"cfg-mute-cost",revoke_cost:"cfg-revoke-cost",draw_cost:"cfg-draw-cost",armor_cost:"cfg-armor-cost",draw_api_base:"cfg-draw-api-base",draw_api_key:"cfg-draw-api-key",draw_model:"cfg-draw-model",draw_proxy:"cfg-draw-proxy",draw_safety_enabled:"cfg-draw-safety-enabled",draw_blocked_words:"cfg-draw-blocked-words"};
const CFG_BOOL=new Set(["draw_safety_enabled"]);
const CFG_TEXT=new Set(["draw_api_base","draw_api_key","draw_model","draw_proxy","draw_blocked_words"]);
function renderConfigForm(){for(const k in CFG_MAP){const e=document.getElementById(CFG_MAP[k]);if(!e)continue;let v=CONFIG[k];if(CFG_BOOL.has(k)){e.checked=!!v;}else if(CFG_TEXT.has(k))v=v||"";else if(k.indexOf("rate")>=0)v=Math.round((v||0)*100);else v=v||0;if(!CFG_BOOL.has(k))e.value=v;}}
